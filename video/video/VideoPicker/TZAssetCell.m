...
//    self.contentView.backgroundColor = [UIColor colorFromName:@"252832"];
    [self setNeedsLayout];
}
...
//#warning  分辨率
        if (asset.duration < 3 || asset.duration > 600 || MAX(assetWidth, assetHeight) > 2560 || MIN(assetWidth, assetHeight) > 1440 ) {
            [self.contentView addSubview:self.videoMaskView];
            self.videoMaskView.frame = self.contentView.frame;
            self.timeLength.alpha = 0.5;
        }else{
            [_videoMaskView removeFromSuperview];
            self.timeLength.alpha = 1.0f;
        }
...
- (UIImageView *)videoMaskView{
    if (_videoMaskView == nil) {
        _videoMaskView = [[UIImageView alloc] init];
//        _videoMaskView.backgroundColor = [UIColor colorFromName:@"181820"];
        _videoMaskView.alpha = 0.7;
    }
...
- (UILabel *)timeLength {
    if (_timeLength == nil) {
        ...
        timeLength.font = [UIFont systemFontOfSize:11.0f];
...
- (void)layoutSubviews {
    ...
    _timeLength.frame = CGRectMake(0, self.tz_height - 17 - 2, self.tz_width - 3, 17);
